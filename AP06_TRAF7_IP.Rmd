...
## Analyzing Jaimeen and Kari IP data with MSstats

EA.hy926 cells? Antibody is from Proteintech (?) Immunogen data says somewhere between 75-420, could be RING or Zinc-finger domains, the alpha helix, or barely beginning of the WD40 domain. MSstats processing.
...
#### Exploring processed IP data

Starting with volcano plots. There are 1,104 proteins per comparison. We have highlighted either genes from a "genes of interest" custom list (red-orange) and anything significant at the 5% FDR level (horizontal line). The sources for candidate complex members below are from [BioPlex](https://bioplex.hms.harvard.edu/explorer/) and [huMAP 3.0](https://www.biorxiv.org/content/10.1101/2024.10.11.617930v1). If you don't see your favorite gene, let Alex know.
...
df_custom_genes <- read.csv("custom_list_df.csv", header = T)
knitr::kable(df_custom_genes)
...
ggplot() +
  ...
  ggtitle("TRAF7 (no '1752) vs. Control IgG")
 # )
...
ggplot() +
  ...
  scale_color_manual(values = c(cbPalette[7], cbPalette[1]))+
    ...
    ylim(0, 3) +
    xlab(paste0("Log2FC(", unique(df_plotme_ip$Label), ")"))+
  ggtitle("TRAF7 + 3nM '1752 vs. Control IgG")
...
ggplot() +
  ...
  scale_color_manual(values = c(cbPalette[7], cbPalette[1]))+
    ...
    ylim(0, 3) +
    xlab(paste0("Log2FC(", unique(df_plotme_ip$Label), ")"))+
  ggtitle("TRAF7 + 30nM '1752 vs. Control IgG")
...
ggplot() +
  ...
  scale_color_manual(values = c(cbPalette[7], cbPalette[1]))+
    ...
    ylim(0, 3) +
    xlab(paste0("Log2FC(", unique(df_plotme_ip$Label), ")"))+
  ggtitle("TRAF7 + 3uM '1752 vs. Control IgG")
...
The first condition, Ab-Control, is the baseline (TRAF7 pulldown, no '1752, over IgG). The y-axis indicates the differences in Log2FC as '1752 concentration increases. As the line trends positively, this indicates an increased abundance in the IP as '1752 concentration increases. A positive trend may indicate a TRAF7 complex member that is associating more with '1752. A negative trend may indicate a dissociating TRAF7 complex member. Any protein that is significantly changing at the 5% FDR at 30 nM dosage as compared to IgG control are highlighted in the first line graph below. They are also shown in the table below the graph. Beneath that, there is a second line graph that highlights any proteins from our custom gene list at the top of this report. The proteins captured are also listed beneath the line graph.
...
## Also add lines that we are interested in from vec_genes_custom

df_layer1 <- df_line[which(df_line$Protein %in% vec_genes_custom),]
df_layer2 <- df_line[which(!df_line$Protein %in% vec_genes_custom),]
...
EA.hy926 cells dosed with 2, 10, and 750 nM '1752 for 2 hours. Is anything changing? We have shown it's proteasome- and E1-independent. 
...
Have captured RHOB and KLF2, could be MEF2 promoter region via ERK5 phosphorylating MEF2C, a potential hypothesis. 

#### Possible bell-curve hypothesis: DUSP1

DUSP1 is a MEK/MAPK phosphatase that is known to negatively regulate the pathway via dephosphorylation. DUSP1 is not changing much in the 10 nM condition, but is at 750 nM. Could this possibly be desphosphorylating MEKK2/3 and/or ERK5, suspected TRAF7 complex members, to down-regulate MEF2 transcription, or KLF2 expression? Could test with [BCI](https://www.selleckchem.com/products/dual-specificity-protein-phosphatase-1-6-Inhibitor-bcl.html). 

## Mary's SEC/PCP data

EA.hy926 cell lysates. Mary mentions a caveat is that this is performed on lysate. EA.hy926 cells (DMSO or 10 nM PF-08031752 for 4 h). No benzonase. "How would the KLF2 activator (1752) change proteoin complexes?" 5,222 total proteins with at least 2 unique peptides. KLF2 not quantified. Lazear et al. 2023 (Cravatt lab) uses an SEC cutoff of 30. 
...
Let's choose a practical score cutoff. What are the SEC scores at each of these quantiles?
...
95 percentile looks reasonable, though much lower than Cravatt score cutoff of >30. 

#### Overlap SEC/PCP data with TRAF7 IP data. 
The following graphs are the same TRAF7 IP-MS volcanoes shown as before but with details from the SEC data, representing a Venn diagram between the IP and SEC datasets. 

**Key**: all points are from the overlap of IP-MS and SEC/PCP datasets. Between the IP-MS (1,104 proteins) and SEC/PCP data (5,222 proteins), the overlap is 745 proteins total. The **diamonds** indicate any SEC score >= 95th percentile. The **color scale** indicates the SEC score. The labels highlight proteins with their SEC scores beneath. There is one interactive plot below, the TRAF7 + 30 nM/Control IgG. 
...
#### Final thoughts
TRIM21 does not seem to differentially pulldown alongside TRAF7, but does appear to have a complex profile altered with '1752; is this an off-target effect? Does disrupting the TRIM21 complex mediate any part of the KLF2 increased expression?

It does look like MAP3K3/MEKK3 is dissociating somewhat from TRAF7, which also may explain the moderate SEC score. 
