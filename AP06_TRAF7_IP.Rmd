...
print(unique(df_layer1[,c("Protein", "Protein.names", "Gene.Names..primary.")]))
```


## Mary's Total Proteome

Is anything changing? Tough to interpret. 2 hour total treatment, and we have shown it's proteasome- and E1-independent. Perhaps pair with RNA-seq? Have captured RHOB and KLF2, likely MEF2 promoter region via ERK5 phosphorylating MEF2C.
...
# ggplotly(
  ...
  geom_point(data = df_layer2, aes(x = log2FC, y = -log10(adj.pvalue), label = Gene.Names..primary.), color = cbPalette[1], alpha = 0.3) +
     geom_point(data = df_layer1, aes(x = log2FC, y = -log10(adj.pvalue), label = Gene.Names..primary.), color = cbPalette[7], alpha = 0.95) +
  geom_hline(yintercept = -log10(0.05)) +
   ggrepel::geom_label_repel(data = df_layer1, aes(x = log2FC, y = -log10(adj.pvalue), label = Gene.Names..primary.), color = cbPalette[7], max.overlaps = 50, force = 2, alpha = 0.6) +
  geom_hline(yintercept = -log10(0.05)) +
    xlim(-3,3) +
    ylim(0, 4)
# )
